...
const Borrower_modal = props => {
  ...
  const btnSelectEmployee = item => {
    ...
    dispatch(change('Edit_Individual_Loan_Form', 'borrower_name', item.customer_nm));
    dispatch(
      change('Edit_Individual_Loan_Form', 'resident_rgst_id', item.resident_rgst_id),
    );
    dispatch(change('Edit_Individual_Loan_Form', 'customer_no', item.customer_no));
  };
...
function Edit_Individual_Loan(props) {
  ...
  const loadData = async () => {
    ...
    if (fileExists) {
      ...
      setBorrowerMap(`/storage/emulated/0/Pictures/RNSketchCanvas/${retrive_loan_data.application_no}MP01.jpg`)
    }
  };
