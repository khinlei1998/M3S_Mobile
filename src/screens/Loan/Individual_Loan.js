...
import Indi_loan_fingerprint from '../../components/Borrower_Fg_Modal';
import Borrower_Fg_Modal from '../../components/Borrower_Fg_Modal';
  ...
  const btnSelectEmployee = item => {
    ...
    dispatch(
      change(
        'Individual_Loan_Form',
        'employee_num',
        item.employee_num ? item.employee_num.toString() : '',
      ),
    );
...
function Individual_Loan(props) {
  ...
  // fg
  const [show_bor_fg_modal, setShowBorFgModal] = useState(false);
  const [timerCount, setTimerCount] = useState(0);
  const [show_borrower_fig, setBorFgImg] = useState(false);
      ...
      if (!saveImageError) {
        ...
        await storeLoanData(loan_data).then(result => {
          if (result == 'success') {
            ...
            ToastAndroid.show(
              'Individual Loan Application added successfully.',
              ToastAndroid.SHORT,
            );
  ...
  const hideBorrFigModal = () => {
    setShowBorFgModal(false);
    setBorFgImg(false);
  };
  ...
  const showBorFigModal = modal_val => {
    setModalContent(modal_val);
    setShowBorFgModal(!show_bor_fg_modal);

    let interval;

    // if (boreower_fg_visible) {
    // Start the countdown when the modal is visible
    interval = setInterval(() => {
      setTimerCount(prevCount => prevCount + 1);
    }, 1000); // Update every second

    // Set a timer to hide the modal after 5 seconds
    setTimeout(() => {
      clearInterval(interval); // Stop the countdown
      setTimerCount(0);
      setBorFgImg(!show_borrower_fig);
      // show image
    }, 3000);
    // }
  };
  ...
  return (
    <>
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
          <View style={{ flex: 1, backgroundColor: '#fff' }}>
            ...
            <Borrower_Sign
              ...
              show_bor_fg_modal={show_bor_fg_modal}
              showBorFigModal={showBorFigModal}
      ...
      <Borrower_Fg_Modal
        show_bor_fg_modal={show_bor_fg_modal}
        hideBorrFigModal={hideBorrFigModal}
        timerCount={timerCount}
        show_borrower_fig={show_borrower_fig}
      ...
      <Borrower_modal
        ...
        setSelectedTspValue={setSelectedTspValue}
      />
